...
!$use omp_lib
implicit none
integer*4, parameter :: NumT=501, NumE=21, NumR=5000, NumNu=7, NumX=51, NumTheta=78, NumZeta=91 
...
Q0=Ls/temp/1.6d-12
Qinj=eta*Q0*(1+tage/tau0)**((braking+1d0)/(braking-1d0))/(1+(tage-t)/tau0)**((braking+1d0)/(braking-1d0))
...
!        flux_syn_tot=0
        ...
        do j=1, NumX-1 !xfrac
            rdis=sqrt(xx(j)**2+dpul**2-2*xx(j)*dpul*dcos(theta(i)))
